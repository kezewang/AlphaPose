...
. /home/keze/torch/install/bin/torch-activate
if [ $? != 0 ] ; then 
    echo "Terminating..." 
    exit 1
fi
...
# CUDA_VISIBLE_DEVICES=${GPU_ID} python video_wrapper.py --inputlist=${LIST_FILE} --inputpath=${INPUT_PATH} --outputpath=${OUTPUT_PATH} --mode=${MODE}
